...
ChessBoard::ChessBoard(PieceValueType existingBoard[BOARD_SIZE][BOARD_SIZE], MoveSimple move) {
...
ChessBoard::ChessBoard(PieceValueType existingBoard[BOARD_SIZE][BOARD_SIZE]) {
    for (int i = 0; i < BOARD_SIZE; i++) {
        std::copy(std::begin(existingBoard[i]), std::end(existingBoard[i]), std::begin(board[i]));
    }
}

bool ChessBoard::isWithinBoard(Coordinate coordinate) const {
...
std::vector<Move> ChessBoard::possibleMovesForRaycaster(Coordinate currentCoordinate,
                                                        PieceType pieceType,
...
std::vector<Move> ChessBoard::possibleMovesForNonRaycaster(Coordinate currentCoordinate,
...
ChessBoard::ChessBoard(std::string boardString) {
...
void ChessBoard::iterateBoard(std::function<void(int16_t, int16_t, PieceValueType)> callback) const {
    for (int i = 0; i < BOARD_SIZE; i++) {
...
std::string ChessBoard::asString() const {
...
ChessBoard ChessBoard::boardByMoving(MoveSimple move) {
    return ChessBoard(board, move);
}

ChessBoard ChessBoard::boardByChangingPieceType(PieceType pieceType, Coordinate coordinate) {
...
std::vector<Move> ChessBoard::possibleMovesForPlayer(uint8_t player) {
    std::vector<Move> possibleMoves{};
